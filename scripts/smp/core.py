...
# Basketball GM's skill rule: a weighted mean of the sub-ratings, over a cutoff.
# code -> (rating keys, weights, cutoff), matching zengm's skills.basketball.ts.
#
# BBGM bakes `skills` into every ratings row it generates, but the SMP II pool was
# assembled from a real-player database that carries none, so the badges have to be
# recomputed here or the cards show nothing. These weights are not guessed: they were
# recovered from the 2,309 ratings rows in the archived SMP I exports (league-data/smp1)
# that BBGM itself labelled, and they reproduce every one of them exactly. Rows for
# seasons before the league started disagree ~10% of the time -- those are imported real
# -player histories whose badges were computed elsewhere, not by this engine.
SKILL_RULES: dict[str, tuple[tuple[str, ...], tuple[float, ...], float]] = {
    "3":  (("oiq", "tp"), (0.1, 1), 0.59),
    "A":  (("stre", "spd", "jmp", "hgt"), (1, 1, 1, 0.75), 0.63),
    "B":  (("drb", "spd"), (1, 1), 0.68),
    "Di": (("hgt", "stre", "spd", "jmp", "diq"), (2.5, 1, 0.5, 0.5, 2), 0.57),
    "Dp": (("hgt", "stre", "spd", "jmp", "diq"), (0.5, 0.5, 2, 0.5, 1), 0.61),
    "Po": (("hgt", "stre", "spd", "ins", "oiq"), (1, 0.6, 0.2, 1, 0.4), 0.61),
    "Ps": (("drb", "pss", "oiq"), (0.4, 1, 0.5), 0.63),
    "R":  (("hgt", "stre", "jmp", "reb", "oiq", "diq"), (2, 0.1, 0.1, 2, 0.5, 0.5), 0.61),
    "V":  (("ins", "dnk", "fg", "tp", "spd", "hgt", "drb", "oiq"),
           (1.5, 1, 1, 1, 0.5, 0.5, 0.5, 0.5), 0.61),
}


def rating_skills(rating: dict[str, Any]) -> list[str]:
    """The skill badges for a ratings row, in SKILL_LABELS order.

    An exported ``skills`` list wins -- BBGM is the authority on its own rows -- and
    is only computed from SKILL_RULES when the export doesn't carry one.
    """
    exported = rating.get("skills")
    if isinstance(exported, list):
        return [str(s) for s in exported]
    out = []
    for code, (keys, weights, cutoff) in SKILL_RULES.items():
        values = [rating.get(key) for key in keys]
        if any(v is None for v in values):
            continue
        total = sum(safe_float(v) * w for v, w in zip(values, weights))
        if total / sum(100 * w for w in weights) > cutoff:
            out.append(code)
    return out
...
def pct_rank(value: Any, ramp: list[float]) -> float | None:
    """Percentile (0-100) of ``value`` within a ``pct_ramp`` column, or None.

    Ties share a percentile (the midpoint of the tied run), so equal ratings always
    get the same number -- and the same color, since heat_style_pct reads this.
    """
    if len(ramp) < 2:
        return None
    ...
    return 100.0 * ((lo + hi) / 2.0) / len(ramp)   # midpoint rank of the tied run
...
def heat_style_pct(value: Any, ramp: list[float]) -> str:
    ...
    """
    pct = pct_rank(value, ramp)
    if pct is None:
        return ""
    hue = 4 + max(0.0, min(1.0, pct / 100.0)) * 126   # 4 = red, 130 = green
    return f"background-color: hsla({hue:.0f}, 55%, 41%, .45)"


def ordinal(n: int) -> str:
    """1 -> '1st'. Used for percentile tooltips."""
